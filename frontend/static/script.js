// Environment configuration - use values from window.env
const config = {
  appHost: window.env.APP_HTTP_HOST || 'localhost',
  appPort: window.env.APP_HTTP_PORT || '3000',
  dashboardHost: window.env.APP_DASHBOARD_HTTP_HOST || 'localhost',
  dashboardPort: window.env.APP_DASHBOARD_HTTP_PORT || '8080',
  tenantId: window.env.TENANT_ID || 'default',
  appName: window.env.APP_NAME || 'oracle-app',
  temporalHost: window.env.TEMPORAL_UI_HOST || 'localhost',
  temporalPort: window.env.TEMPORAL_UI_PORT || '8233'
};
...
// Helper function to build Temporal UI URL
function buildTemporalUrl(path = '') {
  return `http://${config.temporalHost}:${config.temporalPort}${path}`;
}
...
async function performConnectionTest() {
  ...
  let extra = { database };
  if (sqlalchemyUrl) {
    extra.compiled_url = `oracle+oracledb://${sqlalchemyUrl}`;
  }
  ...
  const response = await fetch(buildApiUrl('/workflows/v1/auth'), {
    method: "POST",
    headers: {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set up Temporal UI link using environment variables
  const temporalUILink = document.getElementById("temporalUILink");
  if (temporalUILink) {
    temporalUILink.href = buildTemporalUrl('/namespaces/default/workflows');
  }
...
async function fetchMetadata() {
  try {
    ...
    const response = await fetch(buildApiUrl('/workflows/v1/metadata'), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
async function runPreflightChecks() {
  ...
  try {
    ...
    const response = await fetch(buildApiUrl('/workflows/v1/check'), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    try {
      ...
      const payload = {
        ...
        metadata: {
          ...
          const response = await fetch(buildApiUrl('/workflows/v1/start'), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
